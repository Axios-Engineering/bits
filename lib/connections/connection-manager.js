...
(() => {
  ...
  class ConnectionManager extends CrudManager {
    ...
    update(id, connectionInfo) {
      ...
      .then(() => this.get(id))
      .then((connection) => {
        return connection.Update(this._processConnectionInfo(connection)); // eslint-disable-line new-cap
      })
      .catch((err) => {
    ...
    deactivateConnection(connection) {
      return this.get(connection.id)
      .then((con) => {
        return con.GetSettings() // eslint-disable-line new-cap
    ...
    _connectionAdded(connectionP) {
      return connectionP
      .then((connection) => {
        return connection.GetSettings() // eslint-disable-line new-cap
        .then((settings) => {
          ...
          if (!this._ids[connection.objectPath]) {
            const updateFunc = (values, value1, value2) => {
              ...
              .then((settings) => {
                ...
                return super.update(this._ids[connection.objectPath], {settings: newSettings});
              })
              .catch((err) => {
