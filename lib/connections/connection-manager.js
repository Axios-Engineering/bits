...
(() => {
  ...
  class ConnectionManager extends CrudManager {
    constructor(deviceManager) {
      ...
      this._deviceManager.on('device-activated', (device) => {
        this.__getActiveConnection(device)
        .then((item) => {
          if (item) {
            this.emit('connection-activated', item.settings.connection);
            this._connections[device.id] = item.settings.connection;
          }
        });
      });
      this._deviceManager.on('device-deactivated', (device) => {
        const connection = this._connections[device.id];
        if (connection) {
          this.emit('connection-deactivated', connection);
          delete this._connections[device.id];
        }
      });
    ...
    __getActiveConnection(device) {
      return Promise.resolve()
      .then(() => device.GetAppliedConnection()) // eslint-disable-line new-cap
      .then((item) => {
        if (Array.isArray(appliedConnection) && 0 < appliedConnection.length) {
          const uuid = appliedConnection[0].connection.uuid;
          return this.__getConnectionByUuid(uuid);
        }
      })
      .catch((err) => null);
    }

    __getConnectionByUuid(uuid) {
      return this._items.find((item) => item.settings.connection.uuid === uuid);
    }
    ...
    load(messageCenter, nm) {
      ...
      .then(() => {
        return this._toolbarItemApi.create({
          primary: {
            href: '/elements/network-manager-toolbar/network-manager-toolbar.html',
            tag: 'network-manager-toolbar'
          }
        });
      })
      .then(() => {
        return this._deviceManager.list()
        .then((devices) => {
          return Promise.all(devices.map((device) => {
            return this.__getActiveConnection(device)
            .then((connection) => {
              return {
                id: device.id,
          ...
          .then((active) => {
            Promise.all(active.map((a) => {
              this._connections[a.id] = a.connection;
              this.emit('connection-activated', a.connection);
            }));
          });
        });
      });
    ...
    create(connectionInfo) {
      return Promise.resolve()
        ...
        .then((result) => {
          if (this._ids.hasOwnProperty(result.objectPath)) {
    ...
    update(id, connectionInfo) {
      ...
      .then(() => {
        if (connectionInfo['802-11-wireless-security']) {
          return super.update(id, {secrets: {'802-11-wireless-security': connectionInfo['802-11-wireless-security']}});
        } else {
          return this.get(id);
        }
      })
    ...
    _cleanUpSettings(settings) {
      ...
      if (settings.hasOwnProperty('bridge') && !settings.bridge.hasOwnProperty('stp')) {
        settings.bridge.stp = true;
      }
        ...
        .then((connection) => {
          ...
          .then(() => {
                  ...
                  .then((settings) => {
                    ...
                    if (settings.connection.type === '802-11-wireless') {
                      ...
                      .then(() => {
                        ...
                        // NOTE To future Nic No you cant call getSecrets here. It cause updated to continously be called
                        // Thats why you need the stupid hack on setting the secrets during the update.
                      })
                      .catch((err) => {
                        logger.error('Unable to set secrets', err);
              ...
              if (!this._ids[connection.objectPath]) {
                ...
                return super.update(this._ids[connection.objectPath], Object.assign({uid: connection.objectPath}, connection));
              }
          });
        })
        .catch((err) => {
    ...
    _connectionRemoved(connection) {
      this._chain = this._chain.then(() => {
        return Promise.resolve()
        .then(() => {
          if (this._ids.hasOwnProperty(connection)) {
            return Promise.resolve()
