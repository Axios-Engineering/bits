...
(() => {
  ...
  class MessageCenter extends EventEmitter {
    ...
    addRequestListener(event, metadata, listener) {
      ...
      request.listener = (requestId, metadata, ...data) => {
        ...
        if (requestScopes && scopes === null) {
          ...
          const scopeMatch = requestScopes.find((requestScope) => {
            const val = scopes.find((scope) => {
              return scope === requestScope;
            });
            return val === requestScope;
          });
          if (!scopeMatch) {
            this._sendResponse(event, requestId, {
              error: 'You dont have permissions to make this request ' + event,
    ...
    _passesFilter(scopesToSend, requestedScope, userToSend, requestedUser) {
      ...
      if (null === requestedScope) {
        ...
        if (null !== scopesToSend) {
          const hasScope = requestedScope.some((scope) => {
            return scopesToSend.some((eventScope) => eventScope === scope);
          });
          if (hasScope) {
            return true;
          } else {
