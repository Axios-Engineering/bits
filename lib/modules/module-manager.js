  ...
  class ModuleManager extends EventEmitter {
    ...
    filterModule(module) {
      ...
      return {
        ...
        'module-dependencies': module['module-dependencies'],
    ...
    getModuleInfoFromModulePackage(filepath) {
      return decompress(filepath, {
        filter: (file) => {
          return (path.basename(file.path) === 'module.json' ||
                  path.basename(file.path) === 'package.json');
        }
      }).then((files) => {
        let rawInfo = null;
        for (let i=0; i<files.length; i++) {
          let fname = path.basename(files[i].path);
          // prefer package.json, only use module.json if rawInfo is still 
          // not set
          if (!rawInfo || fname === 'package.json') {
            rawInfo = files[i].data.toString('utf8');
          }
        }
    ...
    checkModuleInfoRequiredParameters(modInfo) {
      ...
      if (!modInfo.version) {
        return Promise.reject(new Error('Module must have a version'));
      }
      if (!semver.valid(modInfo.version)) {
        return Promise.reject(new Error('Module must have a valid version'));
      }
    ...
    _addModuleInfoToModuleList(modInfo) {
      ...
      if (modInfo.isInstalled && !mod.isInstalled) {
        ...
        mod['module-dependencies'] = modInfo['module-dependencies'] || {};
    ...
    _extractInstalledModuleInfo(dir) {
      let pkgInfoPath = path.resolve(dir, 'package.json');
      let modInfoPath = path.resolve(dir, 'module.json');

      return UtilFs.exists(pkgInfoPath)
      .then((pkgJsonExists) => {
        if (pkgJsonExists) {
          logger.debug(`Loading package.json from ${dir}`);
          return UtilFs.readJSON(pkgInfoPath)
          .then((modInfo) => {
          ...
          .catch((err) => {
            logger.warn('Failed to read package.json: %s', err.toString(), {
              directory: dir
            });
          });
          ...
          .then((modInfo) => {
            modInfo.installedDir = dir;
            modInfo.isInstalled = true;
            modInfo['module-dependencies'] = modInfo.dependencies;
            modInfo.dependencies = undefined;
            return this.addModuleInfo(modInfo);
          })
          .catch((err) => {
    ...
    _verifyMinimumBase(mod) {
      ...
      if (mod && mod['upgrade-dependencies'] && mod['upgrade-dependencies']['bits']) {
        return this.getModuleFromName(mod.name)
        .then((base) => {
          const versionString = mod['upgrade-dependencies']['bits'];
          const baseVersion = base.installedVersion;

          const satisfies = semver.satisfies(baseVersion, versionString);
          if (satisfies) {
            return Promise.resolve();
          } else {
    ...
    _extractModulePackageIntoDirectory(archiveFile, installDir) {
      return decompress(archiveFile, installDir)
      .then((files) => {
        let modInfo = null;
        for (let i=0; i<files.length; i++) {
          let fname = path.basename(files[i].path)
          if (!modInfo || (fname === 'package.json')) {
            modInfo = path.resolve(installDir, fname);
          }
        }
    ...
    _checkModuleDependenciesAreLoaded(mod) {
      if ('object' === typeof(mod['module-dependencies']) && null !== mod['module-dependencies']) {
        const depNames = Object.keys(mod['module-dependencies']);

        return depNames.reduce((promise, depName) => {
          return promise
          .then(() => {
            let modName = depName;
            // the module name bits-base is deprecated, but
            // we do it this way so that:
            // a) backwards compatibility is preserved
            // b) the error messages below use the name as found in
            //    module-dependencies
            if (depName == 'bits-base') {
              modName = 'bits';
            }
            const dep = this._getModuleFromName(modName);
            ...
            const depVersion = mod['module-dependencies'][depName];

            if (!semver.satisfies(dep.installedVersion, depVersion)) {
              return Promise.reject(new Error(mod.name + ' dependency "' + depName + '" version does not satisfy requirement.'));
            }
          });
        }, Promise.resolve())
        .then(() => {
    ...
    _callModuleIndexJsUnload(mod) {
      if (mod.indexJs) {
        if ('function' === typeof(mod.indexJs.unload)) {
          return Promise.resolve()
          .then(() => {
            return mod.indexJs.unload(this._base);
          })
          .then(() => {
            return mod;
          });
    ...
    _markModuleUninstalled(mod) {
      ...
      delete mod['module-dependencies'];
      ...
      .then((g) => {
        ...
        for (let i = 0; i < moduleList.length; i++) {
          if (bfdp.hasPathTo(i) && modIndex !== i) {
            ...
            // module.
            let modDepVersion = depParMod['module-dependencies'][depMod.name];
            ...
            deps.push({
              ...
              isVersionSatisified: semver.satisfies(depVersion, modDepVersion),
            });
          }
        }
