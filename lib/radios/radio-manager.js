...
(() => {
  ...
  const sysbus = require('dbus-native').systemBus();
  ...
  _softblockAll() {
    console.log('SOFTBLOCK CALLED IN MANAGER');
    return sysbus.invoke({
    ...
    body: [
       ...
       'WirelessEnabled',
        ['b', false] // variant, 'b' - signature, true - value
    ]
    });
  }

  _unsoftblockAll() {
    console.log('UNSOFTBLOCK CALLED IN MANAGER');
    return sysbus.invoke({
    path: '/org/freedesktop/NetworkManager',
    destination: 'org.freedesktop.NetworkManager',
    interface: 'org.freedesktop.DBus.Properties',
